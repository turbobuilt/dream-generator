...
let updateStatusEndpoint = process.env.NODE_ENV === "development" ? 
    "https://8567-2001-d08-2802-4101-b15f-f35c-fefd-15a0.ngrok-free.app" : 
    "https://dreamgenerator.ai";

let modalEndpoint = "https://noise-destroyer--stable-diffusion-xl-f.modal.run";
...
export async function generateSdxlModal(prompt: string, imgGenRequest: ImageGenerationRequest): Promise<any> {
    try {
        console.log("Generating modal sdxl");
        const response = await axios.post(modalEndpoint, {
            prompt: prompt,
            auth_key: process.env.dream_sdxl_key,
        ...
        imgGenRequest.status = "PREPARING";
