...
export async function likePrompt(req: Request, res: Response) {
    // Extract prompt id from request body
    let { promptId }: { promptId: number } = req.body;

    console.log(req.body);

    try {
        let [[existingLike]] = await global.db.query(`SELECT * FROM PromptLike WHERE prompt = ? AND authenticatedUser = ?`, [promptId, req.authenticatedUser.id]);
        if (existingLike) {
    ...
    let like = new PromptLike({ prompt: promptId, authenticatedUser: req.authenticatedUser.id })
    await like.save();
