...
export async function generateSd3OctoAI(prompt, imgGenRequest: ImageGenerationRequest) {
    ...
    imgGenRequest.model = "sd-3";
    imgGenRequest.status = "IN PROGRESS";
    await imgGenRequest.save();

    (async () => {
        try {
            let result = await octoAi.imageGen.generateSd3({
                prompt: prompt,
                cfgScale: 7,
                height: 1024,
                numImages: 4,
