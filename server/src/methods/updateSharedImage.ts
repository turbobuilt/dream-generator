...
export async function updateSharedImage(req: Request, res: Response) {
    let { id } = req.params as any;

    let sharedImage = new SharedImage();
    console.log("getting");
    await sharedImage.get(id);
    console.log("got shared image")
    let prompt = new Prompt();
    await prompt.get(sharedImage.prompt);
    ...
    if (uploaded) {
        ...
        sharedImage.uploaded = false;
    }
    console.log("saving")
    try {
        await sharedImage.save();
        return res.send({ sharedImage });
    } catch (err) {
        console.error(err);
