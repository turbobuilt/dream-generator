...
function onTouchEnd(event) {
    if (currentVelocity !== 0 && supportsLinear && false) {
        ...
        // console.log(`all ${timeTillStop}s ${getAnimationLinear(timeTillStop)}`);
        // content.value.style.transitionTimingFunction = getAnimationLinear(timeTillStop);
        // content.value.style.transition = `transform ${timeTillStop}s`;
        // content.value.style.transform = `translateY(${currentY + distanceToStop}px)`;
        doScrollMotion();
    }
}
...
<template>
    ...
    <div class="scroll-view" ref="parent" @wheel="onWheel" @scroll="scrolled" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" @touchcancel="onTouchCancel">
