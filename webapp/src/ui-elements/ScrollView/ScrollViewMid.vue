...
let visilityThreshold = window.innerHeight;
...
function onItemResize({ oldHeight, currentHeight, target }: { oldHeight: number, currentHeight: number, target: HTMLElement }, item, index) {
    ...
    if (store.isSafari && numTouches > 0 && item.bottom <= 0) {
...
function doToucheEndCallbacks() {
    // console.log("doing touch end callbacks", onTouchEndCallbacks.length);
...
function scrolled(event) {
    ...
    // console.log("distance from top", distanceFromTop, "top:", props.items[0].top, "timeout", d.loadPreviousTimeout);
    console.log(d.loadPreviousTimeout === null, distanceFromTop < visilityThreshold, props.items.length > 0, props.items[0].top !== undefined);
...
function checkVisibility() {
    ...
    for (let i = 0; i < props.items.length; i++) {
        ...
        console.log(scrollPosition, visilityThreshold, d.minExtent)
...
<template>
    ...
    <div class="scroll-view" ref="parent" @scroll="scrolled" @touchstart="onTouchStart" @touchend="onTouchEnd" @touchcancel="onTouchCancel">
        <div ref="spacer" style="width: 100%;"></div>
        <div class="scroll-content" ref="content">
            <!-- <Item :index="-1" :top="d.minExtent-100">
            ...
            <!-- <Item :index="items.length + 1" :top="visibleItems[visibleItems.length - 1]?.bottom">
                <div class="load-more load-more-end" key="load-more-end">
