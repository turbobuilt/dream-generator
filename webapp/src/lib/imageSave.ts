import { ImageInProgress } from "@/pages/create/createImage";
...
export function deleteImageDb() {
    // completely delete it when account is deleted
    // return indexedDB.deleteDatabase("images");
    return new Promise((resolve, reject) => {
        let request = indexedDB.deleteDatabase("images");
        request.onsuccess = () => {
            resolve(null);
        }
        request.onerror = (err) => {
...
export async function saveImage(data: SavedImageData | ImageInProgress) {
    let db = indexedDB.open("images", 1);
    db.onsuccess = () => {
