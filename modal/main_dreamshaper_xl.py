...
model_dir = Path("sdxl-dreamshaper-1.0")
...
stub = Stub("stable-diffusion-xl-diffusers-dreamshaper", image=sdxl_diffusers_dreamshaper)

with sdxl_diffusers_dreamshaper.imports():
...
class ImageGenerator:
    ...
    def load(self):
        # self.model = StableDiffusionXLPipeline.from_pretrained(model_dir, torch_dtype=torch.float16, variant="fp16", use_safetensors=True)
        self.model = AutoPipelineForText2Image.from_pretrained(model_dir, torch_dtype=torch.float16, variant="fp16")
    ...
    def run(self, prompt, num_inference_steps, guid, update_status_url):
        try:
            requests.post(update_status_url, json={
                "auth_token": os.environ.get('status_update_urt_token'), 
                "status": "IN_PROGRESS",
                "guid": guid,
                "output_url": f""
            })
