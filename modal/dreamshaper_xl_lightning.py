...
def download_models_sdxl_diffusers_dreamshaper_lightning():
...
sdxl_diffusers_dreamshaper_lightning = (
    Image.debian_slim()
    ...
    .run_commands(f"ls ~/")
    .run_function(download_models_sdxl_diffusers_dreamshaper_lightning)
...
app = App("sdxl_diffusers_dreamshaper_lightning", image=sdxl_diffusers_dreamshaper_lightning)

with sdxl_diffusers_dreamshaper_lightning.imports():
...
@app.cls(gpu=modal.gpu.A10G(), enable_memory_snapshot=True, container_idle_timeout=2)
class ImageGenerator:
    ...
    def load(self):
        ...
        import json
        pipe = AutoPipelineForText2Image.from_pretrained(model_id, torch_dtype=torch.float16, variant="fp16", local_files_only=True)
...
def f(data: Dict):
    ...
    num_inference_steps = data.get("num_inference_steps", 6)
