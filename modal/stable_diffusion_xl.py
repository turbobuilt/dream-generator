...
def download_models_stable_diffusion_xl():
...
stable_diffusion_xl = (
    ...
    .run_commands(f"ls ~/")
    .run_function(download_models_stable_diffusion_xl)
...
app = App("stable_diffusion_xl", image=stable_diffusion_xl)

with stable_diffusion_xl.imports():
...
@app.cls(gpu="A100-40GB", enable_memory_snapshot=True, container_idle_timeout=2, timeout=30)
    ...
    def run(self, prompt, num_inference_steps, guid, update_status_url):
        try:
            requests.post(update_status_url, json={
                "auth_token": os.environ['status_update_url_token'], 
                "status": "IN_PROGRESS",
                "guid": guid,
                "output_url": f""
            })
            ...
            print(httpE)
        try:
            print("starting image generate")
            # image = self.model(prompt, num_inference_steps=25).images[0]
            images = self.model(prompt, num_inference_steps=num_inference_steps, guidance_scale=7, strength=.4)
...
def f(data: Dict):
    ...
    if auth_key != os.environ['extra_auth_key']:
