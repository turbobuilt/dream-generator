...
import 'Rendering.dart' show RenderViewport, SliverList;
...
///
class ListItemInfo {
  var isAddedToFront = false;
  var private = false;
  var isStartLoading = false;
  var isStartNoMore = false;
  var isEndLoading = false;
  dynamic item;
  ListItemInfo(
      {this.isAddedToFront = false,
      this.private = false,
      this.item,
      this.isStartLoading = false,
      this.isStartNoMore = false,
...
class ListView extends BoxScrollView {
  ...
  ListView.builder({
    ...
    String? noMoreAtStartText,
    String? noMoreAtEndText,
    super.cacheExtent,
    int? semanticChildCount,
    super.dragStartBehavior,
    super.keyboardDismissBehavior,
    ...
    super.clipBehavior,
        ...
        childrenDelegate = SliverChildBuilderDelegate(
          // custom item builder.  If at start, insert a text that says "no more at start", same if atEnd.  otherwise run builder
          (BuildContext context, int index) {
            if (index == 0) {
              if (loadingStart) {
                return CustomData(
                  data: ListItemInfo(private: true, isStartLoading: true),
                  child: const SizedBox(
                    height: 50,
                    child: Center(child: CupertinoActivityIndicator()),
                  ),
                );
              } else if (atStart || !loadingStart) {
                return CustomData(
                  data: ListItemInfo(private: true, isStartNoMore: true, item: { "addAtFront": true }),
                  child: SizedBox(
                    height: 50,
                    child: Center(child: atStart ? Text(noMoreAtStartText ?? 'No more at start') : const Text("Scroll To Load More")),
                  ),
                );
              }
            }
            if (index == itemCount! + 1) {
              if (loadingStart && itemCount == 0) {
                return CustomData(
                  data: ListItemInfo(private: true, isEndLoading: true),
                  child: const SizedBox(
                    height: 0,
                  ),
                );
              ...
              if (showLoadMore) {
                ...
                } else if (atEnd || !loadingEnd) {
                  return CustomData(
                    data: ListItemInfo(private: true, isEndLoading: true),
                    child: SizedBox(
                      height: 50,
                      child: Center(child: atEnd ? Text(noMoreAtEndText ?? 'No more at end') : const Text("Scroll To Load More")),
                    ),
                  );
                }
              } else {
            ...
            return CustomData(
              data: ListItemInfo(item: item),
              child: itemBuilder(context, itemIndex),
            );
          },
          findChildIndexCallback: findChildIndexCallback,
          childCount: (itemCount ?? 0) + 2,
          addAutomaticKeepAlives: addAutomaticKeepAlives,
          addRepaintBoundaries: addRepaintBoundaries,
          addSemanticIndexes: addSemanticIndexes,
        ),
  ...
  /// }
  ///
  /// class _MyListViewState extends BetterState<MyListView> {
  ...
  /// }
  ///
  /// class _KeepAliveState extends BetterState<KeepAlive> with AutomaticKeepAliveClientMixin{
  ///   @override
  ...
  // Map<dynamic, bool>? previousItemsMap;
  // Map<dynamic, bool> itemsToAddAtStart = {};
  // Map<dynamic, bool>
  // Map<dynamic, bool> itemsToAddAtEnd = {};

  // updateItems() {
  //     //   // previousItems = items;
  //   // return;
  //   if (previousItems?.length == items?.length && previousItems?.firstOrNull == items?.firstOrNull) {
  //     return;
  //   }
  //   if (items == null || items?.isEmpty == true) {
  //     previousItems = items;
  //     previousItemsMap = null;
  //     // itemsToAddAtEnd = {};
  //     itemsToAddAtStart = {};
  //     return;
  //   }
  //   // find items added to start
  //   var atStart = true;
  //   for (int i = 0; i < items!.length; ++i) {
  //     if (previousItemsMap?[items![i]] != true) {
  //       itemsToAddAtStart[items![i]] = true;
  //       continue;
  //     }
  //     atStart = false;
  //     break;
  //   }
  //   Map<dynamic, bool> currentItems = {};
  //   for (int i = 0; i < items!.length; ++i) {
  //     currentItems[items![i]] = true;
  //   }
  //   previousItemsMap = currentItems;
  //   previousItems = items;
  // }
