...
import 'package:dev/widgets/CustomData.dart';
import 'package:dev/widgets/ListView.dart';
...
import 'package:flutter/rendering.dart' hide RenderViewport, RenderViewportBase;
// import 'scroll_delegate.dart' show SliverChildBuilderDelegate;
...
class RenderViewport extends RenderViewportBase<SliverPhysicalContainerParentData> {
  ...
  var fixHeight = false;
  double startLoadingHeight = 0;
  var startLoadingShowing = false;
  ...
  double _attemptLayout(double mainAxisExtent, double crossAxisExtent, double correctedOffset) {
    ...
    var element = childrenInPaintOrder.first as RenderSliverPadding;
    var child = element.child as RenderSliverList;
    var currentChild = child.firstChild as RenderRepaintBoundary?;

    if (currentChild != null && startLoadingHeight == 0) {
      var customData = currentChild.child as RenderCustomData;
      var data = customData.data as ListItemInfo;
      if (data.isStartLoading) {
        startLoadingHeight = customData.height;
      }
    }
    ...
    while (currentChild != null) {
      ...
      if (data.private) {
        ...
        if ((data.item != null && data.item["addAtFront"] == true)) {
          print("adding at fron");
          result -= customData.height;
          data.item.remove("addAtFront");
          if (data.private) {
            data.item = null;
          }
          // if less than scroll height - height of view, limit it
          // get scrol height of child RenderSliverList
          var scrollHeight = child.geometry!.scrollExtent;
          var heightOfView = mainAxisExtent;
          var max = scrollHeight - heightOfView;
          if (result < -max) {
            result = -max;
          }
        } else if ((!data.private && isFirstLoad)) { 
          isFirstLoad = false;
          result -= startLoadingHeight;
        } else {
          break;
        }
      }
      currentChild = child.childAfter(currentChild) as RenderRepaintBoundary?;
    }
    return -result;
  }
...
// extension Freedom on RenderBox {
//   Size get trueSize {
//     // return constraints.biggest;
//     return _size!;
//   }
...
// extension Sizer on RenderSliverList {
//   getChildSizes() {
//     final List<Size> sizes = [];
//     RenderBox? child = firstChild;
//     while (child != null) {
//       child.layout(child.constraints, parentUsesSize: true);
//       sizes.add(child.paintBounds.size);
//       child = childAfter(child);
//     }
//     return sizes;
//   }
